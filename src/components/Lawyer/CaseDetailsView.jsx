...
import { FaArrowLeft, FaCheck, FaCommentDots, FaPencilAlt, FaTimes, FaTrash } from 'react-icons/fa';
import { useSearchParams } from 'react-router-dom';
import api from '../../api/axios';
import { formatTime, toDateInput, toTimeInput } from '../../utils/lawyerUtils';
import CaseFloatingChat from './CaseFloatingChat';
...
const CaseDetailsView = ({
  ...
  const [showCaseChat, setShowCaseChat] = useState(() => searchParams.get('openChat') === 'true');
  ...
  const isTeamCase = displayTeam?.id && displayTeam.id !== 'personal';
  const isCaseOwner = String(caseRecord?.addedBy?._id || caseRecord?.addedBy || caseRecord?.ownerId?._id || caseRecord?.ownerId || '') === String(currentUserId || '');
  const showChatButton = isTeamCase || !isCaseOwner || Boolean(hearingLawyerId);
  ...
  return (
    <div className="space-y-6 rounded-2xl border border-[#d7e9ef] bg-white p-6 shadow-sm relative">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between border-b border-[#eef5f8] pb-5">
        ...
        <div className="flex flex-wrap items-center gap-3">
          {showChatButton ? (
            <button
              type="button"
              onClick={() => setShowCaseChat((prev) => !prev)}
              className="inline-flex items-center gap-1.5 rounded-xl border border-[#e0c765] bg-[#fff9db] px-3.5 py-1.5 text-xs font-bold text-[#8a6d05] shadow-sm transition hover:bg-[#f1d15f] hover:text-zinc-950"
      ...
      <div className="rounded-xl border border-[#d7e9ef] bg-[#f8fbfc] p-5 space-y-4">
        ...
        {localHearingHistory.length === 0 ? (
          <p className="rounded-lg border border-dashed border-[#d7e9ef] bg-white px-4 py-6 text-center text-sm text-[#5f7488]">
              ...
              return (
                <div key={row.id || row.tempKey} className="relative rounded-xl border border-[#d7e9ef] bg-white p-4 shadow-sm">
                  ...
                  <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                    <label className="text-xs font-bold text-[#5f7488]">
                      Court
                      {canEditCase ? (
                        <input
                          type="text"
                          ...
                          className="mt-1 block w-full rounded-md border border-[#d7e9ef] px-3 py-2 text-sm text-[#062552] outline-none focus:border-[#15a276]"
                        />
                      ) : (
                        <span className="mt-1 block text-sm text-[#062552]">{row.courtName || 'Not provided'}</span>
                      )}
                    </label>
                    ...
                    <label className="text-xs font-bold text-[#5f7488]">
                      ...
                      {canEditCase ? (
                        ...
                        <span className="mt-1 block text-sm text-[#062552]">{formatDate(row.hearingDate) || 'Not provided'}</span>
                      )}
                    </label>
                      ...
                      {canEditCase ? (
                        <span className="mt-1 flex gap-2">
                          <input
                            ...
                            className="min-w-0 flex-1 rounded-md border border-[#d7e9ef] px-3 py-2 text-sm text-[#062552] outline-none focus:border-[#15a276]"
                          />
                          <button
                            type="button"
                            onClick={() => handleHearingHistoryChange(index, 'hearingTime', '')}
                            className="rounded border border-[#d7e9ef] px-2 text-xs text-[#5f7488]"
                          >
                            Clear
                          </button>
                        </span>
                      ) : (
                        <span className="mt-1 block text-sm text-[#062552]">{formatTime(`${row.hearingDate}T${row.hearingTime}`) || 'Not provided'}</span>
                      )}
                    </label>
                    ...
                    <label className="text-xs font-bold text-[#5f7488]">
                      ...
                      {canEditCase ? (
                        <input
                          type="date"
                          value={row.nextHearingDate}
                          onChange={(e) => handleHearingHistoryChange(index, 'nextHearingDate', e.target.value)}
                          className="mt-1 block w-full rounded-md border border-[#d7e9ef] px-3 py-2 text-sm text-[#062552] outline-none focus:border-[#15a276]"
                        />
                      ) : (
                        <span className="mt-1 block text-sm text-[#062552]">{formatDate(row.nextHearingDate) || 'Not scheduled'}</span>
                      )}
                    </label>
                    ...
                    <label className="text-xs font-bold text-[#5f7488]">
                      ...
                      {canEditCase ? (
                        <span className="mt-1 flex gap-2">
                          <input
                            type="time"
                            value={row.nextHearingTime}
                            onChange={(e) => handleHearingHistoryChange(index, 'nextHearingTime', e.target.value)}
                            className="min-w-0 flex-1 rounded-md border border-[#d7e9ef] px-3 py-2 text-sm text-[#062552] outline-none focus:border-[#15a276]"
                          />
                          <button
                            type="button"
                            onClick={() => handleHearingHistoryChange(index, 'nextHearingTime', '')}
                            className="rounded border border-[#d7e9ef] px-2 text-xs text-[#5f7488]"
                          >
                            Clear
                          </button>
                        ...
                        <span className="mt-1 block text-sm text-[#062552]">{formatTime(`${row.nextHearingDate}T${row.nextHearingTime}`) || 'Not provided'}</span>
                      )}
                    </label>

                    <label className="text-xs font-bold text-[#5f7488] sm:col-span-2">
                      Hearing details
                      {canEditCase ? (
                        <input
                          type="text"
                        ...
                        <span className="mt-1 block text-sm text-[#062552]">{row.hearingDetails || 'No details added.'}</span>
                      )}
                    </label>
                  </div>
                </div>
      ...
      {/* Floating Instagram-style Case Chat Overlay Window */}
      {showCaseChat ? (
        <CaseFloatingChat
